(function () {
      var utils = JT.utils,
          ...
          UIBase = JT.ui.UIBase,
          domUtils = JT.dom.domUtils;
      var nodeStack = [];

    function TableUI(options) {
    ...
    TableUI.prototype = {
        ...
        initEditorUI:function () {
            ...
            table.addListener('ready', function () {
                //提供getDialog方法
                table.getDialog = function (name) {
    ...
    utils.inherits(TableUI, JT.ui.UIBase);


    var instances = {};


    JT.ui.Table = function (options) {
        var tableInstance = new JT.Table(options);
        //tableInstance.options = tableInstance;
        var oldRender = tableInstance.render;
        tableInstance.render = function (holder) {
            if (holder.constructor === String) {
                tableInstance.key = holder;
                instances[holder] = tableInstance;
            }
            utils.domReady(function () {
                renderUI();
                function renderUI() {
                    new TableUI(tableInstance.options);
                    // if (holder) {
                    //     if (holder.constructor === String) {
     ...
     *
     */
    JT.getTable = function (id, opt) {
        var table = instances[id];
        if (!table) {
            table = instances[id] = new JT.ui.Table(opt);
            table.render(id);
        }
        return table;
    };
    ...
    JT.delTable = function (id) {
        var table;
        if (table = instances[id]) {
            table.key && table.destroy();
            delete instances[id]
        }
    };
